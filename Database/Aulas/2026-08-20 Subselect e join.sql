...
from Cliente c
left join Locacao l on c.CodCli = l.Co
 
--com subselect
select c.cliente,(select count(l.CodLocacao)
				 from Locacao l
				 where c.CodCli = l.CodCdCli
group by c.clienteli) as qtdLocacao
from Cliente c
order by c.cliente
...
from Cliente c
where (select count(l.CodLocacao) 
					from Locacao l 
					where c.CodCli = l.CodCli ) >2
